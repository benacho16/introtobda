...
### Pre-Crisis
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE, results='asis'}
show_data_pc = data.frame(cbind(colnames(pre_crisis_data), round(cor(pre_crisis_data),2)))
m1_pc<-gvisTable(show_data_pc,options=list(width=1920, height=min(400,27*(nrow(show_data_pc)+1)),allowHTML=TRUE))
print(m1_pc,'chart')
```
###Crisis
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE, results='asis'}
show_data_c = data.frame(cbind(colnames(crisis_data), round(cor(crisis_data),2)))
m1_c<-gvisTable(show_data_c,options=list(width=1920, height=min(400,27*(nrow(show_data_c)+1)),allowHTML=TRUE))
print(m1_c,'chart')
```
<br>
...
###Pre-Crisis
```{r echo=TRUE, eval=TRUE, comment=NA, warning=FALSE,error=FALSE, message=FALSE, prompt=FALSE, tidy=TRUE}
Variance_Explained_Table_results_pc<-PCA(pre_crisis_data, graph=FALSE)
Variance_Explained_Table_pc<-cbind(paste("component",1:ncol(pre_crisis_data),sep=" "),Variance_Explained_Table_results_pc$eig)
...
###Crisis
```{r echo=TRUE, eval=TRUE, comment=NA, warning=FALSE,error=FALSE, message=FALSE, prompt=FALSE, tidy=TRUE}
Variance_Explained_Table_results_c<-PCA(crisis_data, graph=FALSE)
Variance_Explained_Table_c<-cbind(paste("component",1:ncol(crisis_data),sep=" "),Variance_Explained_Table_results_c$eig)
Variance_Explained_Table_c<-as.data.frame(Variance_Explained_Table_c)
colnames(Variance_Explained_Table_c)<-c("Component","Eigenvalue", "Percentage_of_explained_variance", "Cumulative_percentage_of_explained_variance")
```
...
###Pre-Crisis
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE, results='asis'}
show_data_pc2 = data.frame(Variance_Explained_Table_pc)
m1<-gvisTable(show_data_pc2,options=list(width=1920, height=min(400,27*(nrow(show_data_pc2)+1)),allowHTML=TRUE,page='disable'),formats=list(Eigenvalue="#.##",Percentage_of_explained_variance="#.##",Cumulative_percentage_of_explained_variance="#.##"))
print(m1,'chart')
```

###Crisis
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE, results='asis'}
show_data_c2 = data.frame(Variance_Explained_Table_c)
m1<-gvisTable(show_data_c2,options=list(width=1920, height=min(400,27*(nrow(show_data_c2)+1)),allowHTML=TRUE,page='disable'),formats=list(Eigenvalue="#.##",Percentage_of_explained_variance="#.##",Cumulative_percentage_of_explained_variance="#.##"))
print(m1,'chart')
```
<br>
...
###Crisis
```{r echo=TRUE, eval=TRUE, comment=NA, warning=FALSE,error=FALSE, message=FALSE, prompt=FALSE, tidy=TRUE}
eigenvalues_c  <- Variance_Explained_Table_c[,2]
```
###Basic Data
...
df           <- cbind(as.data.frame(eigenvalues_c), c(1:length(eigenvalues_c)), rep(1, length(eigenvalues_c)))
colnames(df) <- c("eigenvalues", "components", "abline")
Line         <- gvisLineChart(as.data.frame(df), xvar="components", yvar=c("eigenvalues","abline"), options=list(title='Scree plot', legend="right", width=900, height=600, hAxis="{title:'Number of Components', titleTextStyle:{color:'black'}}", vAxes="[{title:'Eigenvalues'}]",  series="[{color:'green',pointSize:3, targetAxisIndex: 0}]"))
print(Line, 'chart')
```
...
<br>
<br>1. 6 Components
<br>2. Us Notes and Canadian bonds 
<br>3. C1.Bonds
<br>   C2.currencies
<br>   C3.European stock Indexes
<br>   C4.North American stock Indexes
<br>   C5.European rates and bonds
<br>   C6.Oil
<br>   C7.Industrial metals
<br>   C8.Commodities
<br>   C9.High Value metals
<br>   C10.Asian stocks
<br>4. See above
<br>5. During crisis times, assets increase their correlation, thus less componenets explain a higher variability
<hr> 
...
**Your Answers here:**
<br>
<br> 1. I think is clear enough
<br> 2. Changing the freq, changes the volatility, thus the returns
<br>
